...
target_v = 300                  # 电机达到的预定角速度（rad/s）
max_force = 1                # 电机能够提供的力，这个值决定了机器人运动时的加速度，太快会翻车哟，单位N

log_id = p.startStateLogging(p.STATE_LOGGING_VIDEO_MP4, "/Users/huzhangyi/Library/CloudStorage/OneDrive-whu.edu.cn/课内学习/大三上/Robotics/Robotics_Project/robotmove.mp4") # 开启视频记录
for i in range(100):
    p.stepSimulation()
    ...
    location, _ = p.getBasePositionAndOrientation(robot_id) # 获得当前机器人的位置坐标
...
p.stopStateLogging(log_id) # 关闭视频记录
# 断开连接
